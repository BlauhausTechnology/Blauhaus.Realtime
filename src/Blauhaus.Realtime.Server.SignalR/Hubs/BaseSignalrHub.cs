...
namespace Blauhaus.Realtime.Server.SignalR.Hubs
{
    ...
    public abstract class BaseSignalrHub : Hub
    {
        ...
        protected Task<ApiResult> HandleVoidAuthenticatedCommandAsync<TCommand>(TCommand command, Dictionary<string,string> properties) where TCommand : notnull
        {
            return _commandProcessor.HandleVoidAuthenticatedCommandAsync(command, properties, new SignalrClientConnectionProxy(Context));
        }

        protected Task<ApiResult<TPayload>> HandleCommandAsync<TPayload, TCommand>(TCommand command, Dictionary<string,string> properties) where TCommand : notnull
        {
        ...
        protected Task<ApiResult<TPayload>> HandleAuthenticatedCommandAsync<TPayload, TCommand>(TCommand command, Dictionary<string,string> properties) where TCommand : notnull
        {
